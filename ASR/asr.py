...
from typing import Callable, Dict, Generator, Iterable, List, Optional, TextIO, Union, BinaryIO
...
class AudioChunkFileIterator:
    def __init__(
        ...
        self.file_object: Optional[BinaryIO] = open(str(self.input_file), 'rb')
                    ...
                    if result.is_final:
                        if show_intermediate:
                            ...
                            for i, f in enumerate(output_file):
                                f.write("#1# " + transcript + (overwrite_chars if not file_opened[i] else '') + "\n")
                            num_chars_printed = 0
                        else:
